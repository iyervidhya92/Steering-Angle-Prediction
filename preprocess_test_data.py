...
from config import TestConfig, DataConfig
import glob
...
if __name__ == "__main__":
    ...
    filenames = glob.glob(data_path + "/round2/test/center/*.jpg".format(data_path))
    filenames = sorted(filenames)
    out_name = data_path + "/X_test_round2_hsv_gray_diff_ch4".format(data_path)
    X_test = make_hsv_grayscale_diff_data(num_channels)
    np.save(out_name, X_test)
